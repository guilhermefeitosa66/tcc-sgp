...
class LocalsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create local" do
    assert_difference('Local.count') do
      post locals_url, params: { local: { city: @local.city, country: @local.country, name: @local.name, state: @local.state } }
    end
  ...
  test "should update local" do
    patch local_url(@local), params: { local: { city: @local.city, country: @local.country, name: @local.name, state: @local.state } }
